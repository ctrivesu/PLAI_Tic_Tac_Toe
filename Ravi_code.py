...
def max_value(state,a,b):
    ...
    for child in children:
        payoff=max(payoff,min_value(child,a,b))
        if payoff >= b:
            return payoff
        a=max(a,payoff)
    return payoff
...
def min_value(state,a,b):
    ...
    children = state.children()
    for child in children:
        payoff=min(payoff,max_value(child,a,b))
        if payoff <= a:
            return payoff
        b=min(b,payoff)
    return payoff
...
def alpha_beta_minimax(state):
   ...
   if state.is_max_player():
       payoff = max_value(state,-2,2)
       for child in children:
           payoff_child = max_value(child,-2,2)
           if payoff == payoff_child:
               return child
   else:
       payoff = min_value(state,-2,2)
       children = state.children()
       for child in children:
           payoff_child = min_value(child,-2,2)
           if payoff == payoff_child:
               return child
...
def main():
    ...
    is_max_player = True if player == 'X' else False
    state = TicTacToe(board, player, is_max_player)
